...
if [[ "${install_prompt,,}" =~ ^(y|yes)$ ]]; then
    ...
    if [[ "${auth_prompt,,}" =~ ^(y|yes)$ ]]; then
        ...
        msg_ok "Default authentication configured (admin:admin)"
    fi
...
EOF
    ...
    echo -e "${CM} ${GN}Default credentials: ${BL}admin / admin${CL}"
    echo -e "${YW}⚠️  Please change the password after first login!${CL}"
else
